 ...
 */
public class LJParticlesMSDApp extends AbstractSimulation {
  LJParticlesMSD md = new LJParticlesMSD();
  ...
  PlotFrame MSD = new PlotFrame("t", "R^2", "MSD");
  PlotFrame xVelocityAnalytic = new PlotFrame("vx", "P(vx)", "Analytic probability of vx");
  ...
  public void initialize() {
    ...
    xVelocityHistogram.setNormalizedToOne(true);
    
    double min_v = 0.0;
    double max_v = 5;
    
    
    for(double v = min_v; v <= max_v; v += xVelocityHistogram.getBinWidth()) {
    	double value = Math.exp(-1.0 * v*v / (2.0*md.initialKineticEnergy))*Math.abs(v);
    	xVelocityAnalytic.append(0, v, value);
    }
  }
  ...
  public static void main(String[] args) {
    SimulationControl control = SimulationControl.createApp(new LJParticlesMSDApp());

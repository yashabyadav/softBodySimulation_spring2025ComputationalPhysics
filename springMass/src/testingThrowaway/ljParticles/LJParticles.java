...
//package springSphere;
package testingThrowaway.ljParticles;
import java.awt.*;
...
public class LJParticles implements Drawable, ODE {
  ...
  public void setPositions_814() {
	  for(int i =0; i < N; i++) {
		  state[i*4] = Lx*0.5;
		  state[i*4+2] = (i - 0.5)*Ly/N;
		  
		  if(i == 5) {
			  double delta = 1E-5;
			  state[i*4+1] = 1.0 - delta;
			  state[i*4+3] = delta;
		  }
		  else {
			  state[i*4+1] = 1.0;
  ...
  public void setPositions_polygon() {
	  double x0 = 10.0;
	  double y0 = 4.0;
	  
	  double R = 1.0 / 2.0 / Math.sin(Math.PI/N);
	  
	  for(int i = 0; i < N; i++) {
		  state[i*4] = x0 + R*Math.cos(-Math.PI/2 + 2*Math.PI*i / N);
		  state[i*4+2] = y0 + R*Math.sin(-Math.PI/2 + 2*Math.PI*i / N);
  ...
  public void computeAcceleration() {
    for(int i = 0;i<N;i++) {
      ...
      // force upwards if hits boundary
      if(state[4*i+2] <= 0.0) {
    	  ay[i] += 1000.0;
      }
    }
    
    double eqLengthShortSpring = 1.0;
    double eqLengthSecondNeighborsSpring = Math.sin(2.0*Math.PI/N) / Math.sin(Math.PI/N);
    // this only works for even N i think
    double eqLengthLongSpring = 1.0 / Math.sin(Math.PI / N);
    ...
    for(int i = 0;i<N;i++) {
      ...
      while(j >= N) {
    	...
    	// apply spring force between nearest neighbors, to the right
    	double springMag = -springK*(Math.sqrt(r2) - eqLengthShortSpring);
    	
    	double fx = springMag * dx / Math.sqrt(r2);
      double fy = springMag * dy / Math.sqrt(r2);
      ...
      // spring force next nearest neighbor, to the right
      int k = i + 2;
      while(k >= N) {
        k = k - N;
      }
      
      dx = state[4*i]-state[4*k];
	    dy = state[4*i+2]-state[4*k+2];
	    r2 = dx*dx+dy*dy;
	    
	    //System.out.println(Math.sqrt(r2) + " " + eqLengthSecondNeighborsSpring);
      springMag = -springK*(Math.sqrt(r2) - eqLengthSecondNeighborsSpring);
    	
    	fx = springMag * dx / Math.sqrt(r2);
      fy = springMag * dy / Math.sqrt(r2);
        ...
        // apply force across object
	    int indexAcross = i + (int)(N*0.5);
	    while(indexAcross >= N) {
	    	indexAcross -= N;
	    }
	    
	    if(indexAcross > i) {
	    	 //System.out.println(indexAcross + " " + i);
		     dx = state[4*i]-state[4*indexAcross];
		     dy = state[4*i+2]-state[4*indexAcross+2];
		     r2 = dx*dx+dy*dy;
		    
	    	// apply long spring force
	    	//System.out.println(i + " " + j + " " + Math.sqrt(r2) + " " + eqLengthLongSpring);
	    	springMag = -springK*(Math.sqrt(r2) - eqLengthLongSpring);
  ...
  public void step(HistogramFrame xVelocityHistogram, HistogramFrame yVelocityHistogram) {
    odeSolver.step();
  ...
  public void draw(DrawingPanel panel, Graphics g) {
    ...
    for(int i = 0;i<N;i++) {
      ...
      int xpix_neighbor = panel.xToPix(state[4*neighbor]);
      int ypix_neighbor = panel.yToPix(state[4*neighbor+2]);
      
      g.fillOval(xpix, ypix, 2*pxRadius, 2*pyRadius);
      g.drawLine(xpix + pxRadius, ypix + pxRadius, xpix_neighbor, ypix_neighbor);
      ...
      int xpix_across = panel.xToPix(state[4*indexAcross]);
      int ypix_across = panel.yToPix(state[4*indexAcross+2]);
      
      g.drawLine(xpix + pxRadius, ypix + pxRadius, xpix_across, ypix_across);

...
public class LJParticlesApp extends AbstractSimulation {
  ...
  HistogramFrame yVelocityHistogram = new HistogramFrame("vy", "H(vy)", "Velocity histogram");
  ...
  PlotFrame analyticSpeed = new PlotFrame("speed", "analytic probability", "Analytic speed");
  int overallStep;
  ...
  public void initialize() {
    ...
    speedHistogram.setBinWidth(2*md.initialKineticEnergy/md.N);
    
    double minVal = 0.0;
    double maxVal = 10.0;
    double step = 2*md.initialKineticEnergy/md.N;
    
    for(double i = minVal; i <= maxVal; i += step) {
    	analyticSpeed.append(0, i, Math.exp(-i*i/(2.0*md.initialKineticEnergy)) * i);
    }
    overallStep = 0;
   ...
   */
  public void doStep() {
	md.step(xVelocityHistogram, yVelocityHistogram);
    for(int i = 0; i < md.N; i++) {
    	speedHistogram.append(Math.sqrt(md.state[4*i+1]*md.state[4*i+1] + md.state[4*i+3]*md.state[4*i+3]));
    }
    
    overallStep++;
    
    /*if(overallStep == 1000) {
    	md.odeSolver.setStepSize(-md.odeSolver.getStepSize());
    }
    else if(overallStep == 2000) {

...
public class softBallNoCenterNode implements Drawable,ODE2D{
    public double x, y;
    vec2 center = new vec2(x,y);
    vec2 initVel = new vec2(x,y);
    ...
    public double[] surfaceSpringsEquilibriumLen;
    ...
    public softBallNoCenterNode(vec2 centerCoordinateVec2, int numBeads, double springConstBlue, double springConstGreen, double springConstArea, double ballRadius, double airDragFactor, double surfaceSpringDampingConst, double centerToSurfaceSpringDampingConst, boolean displayForceVectors){
    ...
    public void initializeSoftBall(){
        this.stateVec = new vec2[2*(this.n)+1];//n position vectors for the surface nodes and 1 for the center of mass 
        ...
        this.allSpringsEquilibriumLen = new double[this.n][this.n-1];
        ...
        /*for(int i =0;i< this.n ;i++){
            for(int j = 0;j<this.n-1;j++){
                ...
                this.allSpringsEquilibriumLen[i][j] = calcCurrSpringLen(this.stateVec[2*i], this.stateVec[2*k]);
            }
        }*/
        ...
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n - 1; j++) {
                ...
                double ang = (j + 1) * 2.0 * Math.PI / n;
                allSpringsEquilibriumLen[i][j] = R * 2.0 * Math.sin(ang / 2.0);
            }
        }
        //ai genereated------------------------------//
        ...
        //System.out.println(this.innerSpringsEquilibriumLen);
        for(int i = 0; i< this.allSpringsEquilibriumLen.length;i++){
            for(int j = 0; j< this.allSpringsEquilibriumLen[i].length;j++){
                //if (i == 0) System.out.println("i = "+i+", j = "+j+", innerSpring  = "+this.allSpringsEquilibriumLen[i][j]);
            }
        }
        //System.out.println("_______________");
    ...
    public void computeAcceleration(){
        ...
        //System.out.println(this.allSpringsEquilibriumLen[0].length);
...
//            if ( i== 2 ) System.out.println("timestep = "+this.stateVec[this.stateVec.length-1].x+"_______________________");
            ...
            //System.out.println(this.allSpringsEquilibriumLen[i][0]);
            //System.out.println(this.allSpringsEquilibriumLen[i][this.n-2]);

            fiSpring.addSelf(calcSpringForceVector(this.allSpringsEquilibriumLen[i][0], springLenNext, this.k, nHatNext, this.stateVec[2*i+1], this.stateVec[2*l+1], this.cSurface));
            //if(i == 3) System.out.println("Prev del L: "+(springLenPrev-this.allSpringsEquilibriumLen[i][0]));
            //if(i == 3) System.out.println("Next del L: "+(springLenPrev-this.allSpringsEquilibriumLen[i][this.n-2]));
            //System.out.println("F_prev: "+fiSpring.x+","+fiSpring.y);
            fiSpring.addSelf(calcSpringForceVector(this.allSpringsEquilibriumLen[i][this.n-2], springLenPrev, this.k, nHatPrev, this.stateVec[2*i+1], this.stateVec[2*j+1], this.cSurface));
            //if(i == 3) System.out.println("f_"+i+j+"+f_"+i+l+" = "+fiSpring.x+","+fiSpring.y);


            int loopVar2 = 1;
//            if (i == 0) System.out.println("leqbm_"+i+0+"="+this.allSpringsEquilibriumLen[i][0]);
            for (int loopVar = j+1; loopVar < l; loopVar++) {
                if (loopVar == i || loopVar == (i+1)%this.n || loopVar == (i+this.n-1)%this.n) continue; // skip immediate neighbors
                
                //System.out.println("loopVar ="+loopVar+" loopVar2="+loopVar2+" i ="+i+" j="+j+" l="+l);
//                if (i == 0) System.out.println("loopVar="+loopVar);
//                if (i == 0) System.out.println("loopVar2="+loopVar2);
                double l_eqbm = this.allSpringsEquilibriumLen[i][loopVar2];

//                if (i == 0) System.out.println("leqbm_"+i+loopVar2+"="+l_eqbm);
                
                
                //System.out.println("loopVar ="+loopVar+" loopVar2="+loopVar2+" i ="+i+" j="+j+" l="+l);
                //if (loopVar == i || loopVar == j || loopVar == l) continue;


                double l_curr = calcCurrSpringLen(this.stateVec[2*i], this.stateVec[2*loopVar]);
                //if (i==0) System.out.println("l_curr="+l_curr);
                ...
                vec2 fij = calcSpringForceVector(l_eqbm, l_curr, this.k2, nHat, vel_i, vel_j, this.cInner);

                //if(i == 0) System.out.println("del L_"+i+loopVar+"="+(l_curr-l_eqbm));

                //if(i == 0) System.out.println("f_"+i+loopVar+" = "+fij.x+","+fij.y);
                //if(i == 3) System.out.println("f_"+i+loopVar+" = "+fij.x+","+fij.y);
                fiSpring.addSelf(fij);



                loopVar2++;
                if(loopVar2 > this.n-1){
                    break;
                    //loopVar2 = 1;
                    //continue;
                }
            }
//            if (i == 0) System.out.println("leqbm_"+i+0+"="+this.allSpringsEquilibriumLen[i][this.n-2]);
            //System.out.println("F_prev+F_next+F_inner: "+fiSpring.x+","+fiSpring.y);
            fiSpring.addSelf(this.g);
            //System.out.println("i="+i);
            //System.out.println("F_prev+F_next+F_inner+F_g: "+fiSpring.x+","+fiSpring.y);
    ...
    public void getRateVec(vec2[] stateVec, vec2[] rateVec){
        ...
        /*vec2 vcm = new vec2(0,0);
        for (int i=0; i<this.n;i++){
            vcm.addSelf(this.stateVec[2*i+1]); // "COM" velocity
        }
        vcm.scalerMult(1/this.n);
        */
        for(int i=0;i<this.n;i++){
    ...
    public void draw(DrawingPanel panel, Graphics g){
        ...
        g2.drawRect(panel.xToPix(-3), panel.yToPix(-3), 200,10);
        ...
        for(int i=0; i<this.n; i++){
            ...
            //g2.setColor(Color.BLACK);
            g2.setColor(azure.get(3));
            g2.drawLine(xipix, yipix, xkpix, ykpix);

            //g2.setColor(Color.RED);
            g2.setColor(metro.get(1));
